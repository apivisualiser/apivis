    ...
    {#each _.compact(tabs) as tab, index}
      <div class="container" class:isInline class:tabVisible={index == value} style:max-width={containerMaxWidth}>
        ...
        {#if tab.slot != null}
          ...
          {:else if tab.slot == 2}<slot name="2" {...tab.props} />
          {:else if tab.slot == 3}<slot name="3" {...tab.props} />
          {:else if tab.slot == 4}<slot name="4" {...tab.props} />
          {:else if tab.slot == 5}<slot name="5" {...tab.props} />
          {:else if tab.slot == 6}<slot name="6" {...tab.props} />
          {:else if tab.slot == 7}<slot name="7" {...tab.props} />
          {/if}
        {/if}
      </div>
    {/each}
